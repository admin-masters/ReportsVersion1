...
def campaign_overview(request, brand_campaign_id: str | None = None):
    ...
    action_panel = {
        ...
        "actions": [
            "Continue current execution and monitor weekly movement.",
            "Review low performing cohorts and fine tune content messaging.",
        ],
    }
    collateral_cards = {
        "current": {},
            ...
            if schema_rows:
                ...
                schedule_rows = _fetch_dicts(
                    ...
                    JOIN silver.map_brand_campaign_to_campaign m
                      ON m.campaign_id_resolved = s.campaign_id
    ...
    reached_pct_series = [_to_float(r.get("reached_pct_of_total")) * 100 for r in weekly_rows]
    opened_pct_series = [_to_float(r.get("opened_pct_of_reached")) * 100 for r in weekly_rows]
    pdf_pct_series = [_to_float(r.get("pdf_download_pct_of_opened")) * 100 for r in weekly_rows]
    video_pct_series = [_to_float(r.get("video_50_pct_of_opened")) * 100 for r in weekly_rows]
